...
const store = new Vuex.Store({
    ...
    state: {
        origin: window !== top && getQueryVariable('origin'), // 'miniTrade' ||
    ...
    mutations: {
        ...
        UPDATE_mobilePhonePrefix (state, data) {
            state.mobilePhonePrefix = data
            localSet('mobilePhonePrefix', data)
        }
    },
    actions: {
        login ({ dispatch, commit, state }, data = {}) {
            ...
            if (data.accountNo && !data.accountNo.includes('@') && !data.mobilePhonePrefix) {
                data.mobilePhonePrefix = state.mobilePhonePrefix
            }
            return login(data)
                .then(async res => {
                    loading.clear()
                    if (parseInt(res.code) === 1) {
                        ...
                        if (data.accountNo) {
                            ...
                            if (window.isPC) {
                                if (data.accountType === 'real') dispatch('getMsgTypeList')
                            } else {
                        ...
                        if (resultData.uiPageList && resultData.uiPageList.country) {
                            // 配置区号
                            const { uiModules = [] } = resultData.uiPageList.country || {}
                            const localCode = localGet('mobilePhonePrefix')
                            const isValid = uiModules.find(_ => _.img === localCode)
                            if (isValid) {
                                commit('UPDATE_mobilePhonePrefix', localCode)
                            } else {
                                commit('UPDATE_mobilePhonePrefix', '86')
                            }
                        } else {
                            // 未配置区号
                            commit('UPDATE_mobilePhonePrefix', '86')
        ...
        getGeolocation ({ commit, state }) {
            getGeolocation()
                .then(geolocation => {
                    commit('UPDATE_userRiskInfo', { ...state.userRiskInfo, geolocation })
                })
        },
