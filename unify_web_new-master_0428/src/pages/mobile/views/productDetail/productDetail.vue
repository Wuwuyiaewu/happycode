<template>
  ...
  <div
    class="productDetail"
    :style="origin === 'miniTrade' && isExperience && 'padding-top: 0'"
  >
    ...
    <section ref="container" class="container">
      <div v-if="product" ref="productInfo" class="productInfo">
        <div class="hd">
          <div
            class="hd-left"
            :class="origin === 'miniTrade' && isExperience && 'flex-auto'"
          >
          ...
          <div v-if="!(origin === 'miniTrade' && isExperience)" class="others">
            ...
            <div class="others-bottom">
              ...
              <div v-if="product.groupSymbol.expiry_time" class="expire-date">
                ...
                <span class="value">
                  <template>{{
                    (product.groupSymbol.expiry_time * 1000)
          ...
          <div
            v-if="origin === 'miniTrade' && isExperience"
            class="account-info"
            @click="() => (isAccountInfo = !isAccountInfo)"
          >
            <span class="text">
            ...
            <div class="balance">
              ${{
                $store.state.ix_user.userLoginInfo.balance
                  | moneyAccuracy(false)
              }}
            </div>
          </div>
        ...
        <div v-if="!(origin === 'miniTrade' && isExperience)" class="bd">
          <div class="item">
            <p class="priceBottom">
              ...
              <span>{{
                (product.lastPrice && product.lastPrice.open_price)
                  | priceDigit(digit)
              }}</span>
            </p>
            <p>
              ...
              <span>{{
                (product.lastPrice && product.lastPrice.yesterday_price)
                  | priceDigit(digit)
              }}</span>
            </p>
          </div>
          ...
          <div class="item">
            ...
            <p v-if="product.low_price">
              ...
              <span>
                <template v-if="volumePips">
                  {{ volumePips | priceDigit }}
                  {{ product.groupSymbol.profit_currency }}
                </template>
              </span>
            </p>
          </div>
      ...
      <div class="positionList">
        ...
        <template v-if="position.total > 0">
              ...
              <div class="left">
                ...
                <div class="amount">
                  ...
                  <span
                    :class="{
                      riseColor:
                        item.profitOrLoss + item.swap + item.commission > 0,
                      fallColor:
                        item.profitOrLoss + item.swap + item.commission < 0,
                    }"
                  >
                    {{
                      (item.profitOrLoss + item.swap + item.commission)
                        | moneyAccuracy
                    }}
                  </span>
                </div>
          ...
          <a
            v-if="position.total > 3"
            class="more"
            href="javascript:void(0)"
            @click="toPositionList"
          >
            <span>
        ...
        <div v-else-if="position.total === 0" class="noDataWrap">
          <ListEmpty
            icon-size="44px"
            :txt="
              $t(isGuest ? 'trade.positionForGuestTip' : 'trade.noPositions')
            "
          />
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    position() {
      const positions = this.accountData.positionList.filter(
        (item) => item.code_id == this.productId
      );
      const data = {
        flg: {
          last: -1,
    ...
    digit() {
      ...
      return product.groupSymbol && product.groupSymbol.hasOwnProperty("digits")
        ? product.groupSymbol.digits
        : product.digit;
    },
    priceTime() {
      const product = this.product;
    ...
    // 是否为自选产品
    isSelfSymbol() {
      return this.selfSymbolList.find(
        (el) => el.symbolId === parseInt(this.productId)
      );
    },
    isGuest() {
      return !getLoginData();
    ...
    containsWatch() {
      return (
        this.ad.uiModules &&
        this.ad.uiModules.find((el) => el.img.includes("/watch/record"))
      );
    },
    // 振幅
    amplitude() {
      const product = this.product;
      return toFixed(
        (product.high_price - product.low_price) / product.groupSymbol.pip,
        String(this.product.groupSymbol.pips_ratio).length - 1
      );
    },
    productInfo() {
      if (this.$route.params && this.$route.params.id) {
        return (
          this.$store.state.ix_quote.product_map[this.$route.params.id] || {
            groupSymbol: {},
          }
        );
      } else {
  ...
  methods: {
    ...
    init() {
      ...
      this.$store.commit(
        "ix_quote/UPDATE_PRODUCT_ACTIVATED_ID",
        this.productId
      );
    },
    toPositionList() {
    ...
    toOrder(direction) {
      ...
      if (this.$route.name === "HomeProduct") {
        this.$router.push({ query: { direction } });
        return;
      }
      const product = this.product;
      ...
      const LPJump = () => {
        const path = "/order";
        window.top.location = `${location.protocol}//${
          location.host
        }/${window.htmlKey +
          path}/${id}?direction=${direction}&t=${encryptLoginData()}`;
      };
      ...
      if (!groupSymbol || !groupSymbol.name) {
        //  || groupSymbol.tradable !== 1 暂时去掉tradable判断，ix系统没有
        ...
        return this.$alert(
          this.$t("productStatus." + groupSymbol.enable) +
            this.$t("productStatus.linkService")
        );
      }
    ...
    fullToPath(positionId) {
      this.showActivityProcess().then(() => {
        ...
        window.top.location = `${location.protocol}//${
          location.host
        }/${window.htmlKey + path}?close=${positionId}&t=${encryptLoginData()}`;
      });
    },
    sell(data) {
      this.symoblCanTrade(data.groupSymbol.id)
        .then((res) => {
          if (!res.mayTrade) {
            this.$toast({
              duration: 2000,
              message: this.$t("trade.marketIsClosed"),
            });
            return;
          }
    ...
    calculatHeight() {
      ...
      this.chartHeight =
        ...
        top.clientHeight -
        productInfo.clientHeight -
        footerBtnBox.clientHeight -
        100;
      this.showChart = true;
    },
    toggleSelf(code_id) {
        ...
        submit
          .then((res) => {
            if (parseInt(res.code) === 1) {
              const msg = this.$t(
                this.isSelfSymbol
                  ? "optional.cancelSuccess"
                  : "optional.addSuccess"
              );
              this.$store.dispatch("getSelfSymbolList").then(() => {
    ...
    toContractInfo() {
      ...
      if (!this.profitToUSD.buy_price)
        this.$ws.send_addSubscription_proList([
          this.productId,
          this.profitToUSD.code_id,
        ]);
    ...
    toWatch() {
      ...
      if (!this.$store.state.ix_user.userLoginInfo.account_id) {
        const cb =
          `/nest/queryinfo?hidehead=1&title=${this.$t("trade.watch")}&url=` +
          encodeURIComponent(url);
        this.$router.push({
          name: "Login",
          query: { cb: encodeURIComponent(cb) },
        });
        return false;
      }
      this.$router.push({
        name: "Nest",
        params: { id: "queryinfo" },
        query: { url, title: this.$t("trade.watch"), hidehead: 1 },
      });
    },
