<template>
    <div class='registerFail' :class='{ pc:isPC }'>
        <a v-if='isPC' class='close' href='javascript:void(0);' @click='goBack'>
            <i class='icon_icon_close_big'></i>
        </a>
        <div class='container'>
            <a v-if='!isAPP && !isPC' class='close' href='javascript:void(0);' @click='goBack'>
                <i class='icon_icon_close_big'></i>
            </a>
            <i class='failIcon'></i>
            <p class='failText'>
    ...
    methods: {
        ...
        tryAgain () {
            if (this.openType === 'openRealAccount') {
                this.$prophet(['trackEvent', '注册', '手机注册', '手机注册_再试试', 12])
            } else if (this.openType === 'RegisterByEmail') {
                this.$prophet(['trackEvent', '注册', '邮箱注册', '邮箱注册_再试试', 20])
            }
