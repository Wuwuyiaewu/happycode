<template>
    <div class='iframeWrapper'>
        ...
        <my-iframe
            ...
            @onerror='hiddenLoading'
            @onload='hiddenLoading'
        />
    </div>
</template>
...
export default {
    ...
    beforeRouteEnter: (to, from, next) => {
        delayAwait(
            () => JSON.parse(sessionStorage.getItem('getAppProperties')),
            true
        ).then((res) => {
            if (!res) next()
            const uiPageList = res.uiPageList
            const type =
                to.params.id === 'opendemo' ? 'openDemoAccount' : 'openRealAccount'
            const openAccount =
                uiPageList.find((el) => el.code === type) ||
                uiPageList.find((el) => el.code === 'RegisterByEmail')
            next(
                openAccount.type === 'interior'
                    ? { name: 'Register', params: { type: openAccount.code, step: 1 } }
                    : true
            )
        })
    },
    ...
    computed: {
        ...mapGetters(['style']),
        iframeUrl () {
            const data = this.$store.state.ix_baseInfo.companyInfo.uiPageList || {
                openRealAccount: {},
                openDemoAccount: {},
            }
            return {
