    ...
    new Vue({
                ...
                data() {
                    return {
                        ...
                        carouselData: [],
                        // 03 總產品清單 json url
                        jsonUrl: './promote/lp336TR/js/code_03_ix.json',
                        // json資料存放陣列
                        allJson: [],
                        // Http 登入獲取 token
                        Http_config: {
            ...
            Wsevent(e) {
                ...
                if (e.toString().indexOf("p(") > -1) {
                    ...
                    let profit_get_sel = document.getElementById('profitGet')
                    switch (ws_realTimeProduct[0]) {
                        case '573004':
                            console.log('黃金');
                            profit_get_sel.textContent = (500 * (ws_realTimeProduct[5] - ws_realTimeProduct[6]) / (ws_realTimeProduct[3] * 0.0025)).toFixed(0)
                            break;
                        case '573139':
                            console.log('欧美');
                            profit_get_sel.textContent = (500 * (ws_realTimeProduct[5] - ws_realTimeProduct[6]) / (ws_realTimeProduct[3] * 0.002)).toFixed(0)
                            break;
                        case '573032':
                            console.log('美日');
                            profit_get_sel.textContent = (500 * (ws_realTimeProduct[5] - ws_realTimeProduct[6]) / (ws_realTimeProduct[3] * 0.002)).toFixed(0)
