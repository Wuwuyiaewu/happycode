var vm = new Vue({
            ...
            data() {
                return {
                    ...
                    // 最後一口價指定 ID
                    lastSelIdOn_Off: LASTPRICE_SEL_ID,
                    // 總產品_最後一口價
                    allLastProduct: [],
                    // 實時報價 開關
            ...
            computed: {
                html() {
                    let vm = this
                    let newSort = vm.allLastProduct.sort((a, b) => {
                        let one = (((a.cur_price - a.yesterday_price) / a.yesterday_price) * 100).toFixed(2)
                        let two = (((b.cur_price - b.yesterday_price) / b.yesterday_price) * 100).toFixed(2)
                        return two - one
                    });
                    return newSort
                }
            ...
            switch (msg_code) {
                ...
                case "InitProductInfo":
                    ...
                    if (vm.lastPriceOn_Off) {
                        // 訂閱所有產品最後一口價
                        if(vm.lastSelIdOn_Off.length === 0){
                            console.log(vm.lastSelIdOn_Off.length);
                            _content_ls = {
                                code_ids: vm.typeId,
                            }
                        }else{
                            _content_ls = {
                                code_ids: vm.lastSelIdOn_Off,
                            }
                        }
                        // --最後一口價
                        vm.WsLastpriceBuildup("lastPrice", _content_ls)
                ...
                case "LastPrice":
                    ...
                    vm.allJson.forEach(el_1=>{
                        vm.allLastProduct.forEach(el_2=>{
                            if((el_1.id*1) === (el_2.code_id*1)){
                                el_2.name = el_1.name
                                el_2.type = el_1.type
                                el_2.en = el_1.en
                                el_2.type = el_1.type
                                el_2.delta = el_1.delta
                            }
                        })
                    })
                    console.log(JSON.parse(JSON.stringify(vm.allLastProduct)));
