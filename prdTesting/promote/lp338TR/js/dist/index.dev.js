...
(function () {
  ...
  function init() {
    ...
    $('.info-name').text(productName); // 最后一口报价
    ...
    getProductSubscription(function (data) {
      curColor = cur === data[3] * 1 ? curColor : cur > data[3] * 1 ? fallColor : riseColor; // buyColor = buyPrice * 1 !== data[2] * 1 ? (buyPrice < data[2] ? riseColor : fallColor) : buyColor
      ...
      color = upDownAmount > 0 ? riseColor : upDownAmount < 0 ? fallColor : '';
      mountedData(); // if (klineDataList.datas) {
    ...
    getKline(function (list) {
      ...
      console.log('getKline: ', klineDataList); // localStorage.setItem('a', JSON.stringify(klineDataList))
  ...
  function mountedData() {
    ...
    }).attr('href', origin + '/yz352001/order/' + productId + '?direction=sell&utm_source=xxss&openbrowser=true&experience=true').find('.sellPrice').text(sellPrice ? (sellPrice * 1).toFixed(digits) : '- -');
    $('.buyBtn').css({
      background: buyColor
    }).attr('href', origin + '/yz352001/order/' + productId + '?direction=buy&utm_source=xxss&openbrowser=true&experience=true').find('.buyPrice').text(buyPrice ? (buyPrice * 1).toFixed(digits) : '- -');
  }
})();