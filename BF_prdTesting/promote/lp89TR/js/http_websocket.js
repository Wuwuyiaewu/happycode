...
const PRODUCT_SEL_ID = ['573004', '573139', '573032']
    // 最後一口價開關 (若 FALSE 則使用指定產品ID)
const LASTPRICE_ON_OFF = true
    // 指定所有產品 ID
const LASTPRICE_SEL = ['573139', '573039', '573024', '573031', '573032', '573043', '573042', '573022', '573017', '573015', '573013', '573014', '573010', '573005', '573004']
    // 輪播 開關
...
const fallColor = 'cRed';
const riseColor = 'cGreen';
var vm = new Vue({
            ...
            data() {
                return {
                    ...
                    carouselData: [],
                    // 03 總產品清單 json url
                    jsonUrl: 'public/js/json/code_BF_ix.json',
                    // json資料存放陣列
                    allJson: [],
                    // Http 登入獲取 token
                    Http_config: {
        ...
        gotoTR(id){
                ...
                let trType = url.searchParams.get("TR_type") == null ? '' : url.searchParams.get("TR_type");
                let homeUrl = origin + "/JW666key/order/"+id+"?direction=buy"+"&utm_source=" + utm_source + "&utm_medium=" + utm_medium + "&utm_term=" + utm_term + "&utm_campaign=" + utm_campaign + "&utm_content=" + utm_content + "&openbrowser=false&experience=true&path=K18";
                $('.TrBtn').attr('href', homeUrl)
    
                // let mineUrl = origin + "/JW666key/mine?utm_source=" + utm_source + "&utm_medium=" + utm_medium + "&utm_term=" + utm_term + "&utm_campaign=" + utm_campaign + "&utm_content=" + utm_content + "&openbrowser=false&experience=true&path=K18";
                // $('.mineBtn').attr('href', mineUrl)
            console.log('666');
        },
        ...
        WsUpdateReal(data) {
            let vm = this
            // 實時數據資料
            vm.commodity.filter(el => {
                if (el.id == data[0]) {
                    el.sell_price = data[1]
                    el.buy_price > data[2] ? el.color_recognition = fallColor : el.color_recognition = riseColor
                    el.buy_price = data[2]
                    el.cur_price = data[3]
                    el.time = data[4]
            ...
            switch (msg_code) {
                ...
                case "InitProductInfo":
                    ...
                    if (vm.lastPriceOn_Off) {
                        ...
                        console.log('最後一口價開關狀態: 全選');
                        vm.WsLastpriceBuildup("lastPrice", _content_ls)
                    } else {
                        _content_ls = {
                            code_ids: vm.selTypeId,
                        }
                        console.log('最後一口價開關狀態: 可選擇產品 ID');
                        vm.WsLastpriceBuildup("lastPrice", _content_ls)
                    }
                    // 輪播 ID 訂閱
                    if (vm.loopOn_Off) {
                ...
                case "LastPrice":
                    ...
                    const method = (el) => {
                        let cur_price = vm.allLastProduct.filter(el2 => {
                            if (el.id == el2.code_id) {
                                return [el2.cur_price, el2.high_price, el2.low_price,el2.buy_price]
                            }
                        })
                        console.log(cur_price[0]);
                        el.buy_price = cur_price[0].buy_price
                    ...
                    commodity.forEach(el => {
                        method(el)
                    })
                    vm.commodity = commodity
                    StockIndex.forEach(el => {
                        method(el)
                    })
                    vm.StockIndex = StockIndex
                    Fe.forEach(el => {
                        method(el)
                    })
                    vm.Fe = Fe
